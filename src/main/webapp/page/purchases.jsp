...
<!-- 采购原材料 -->
	<script>
		...
		layui.use([ 'laydate', 'element', 'table' ], function() {
			...
			table.on('tool(user)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
				...
				if (layEvent === 'del') { //删除
					...
					$.post(url, datas, function(returnData) {
						...
						layer.open({
							...
							btnAlign : 'c',
							moveType : 1, //拖拽模式，0或者1
							content : '<center><div style="width:580px;"><table class="layui-table" id="adc">' + '<thead>'
									+ '<tr>' 
									+ '<th>采购人</th>' 
									+ '<th>材料</th>'
									+ '<th>采购时间</th>' 
				...
				}else if(layEvent === 'pdt'){
					addOrUpdata('重新申请', data.purcId);
				}
			});
	...
	<script>
		...
		layui.use([ 'laydate', 'element', 'table' ], function() {
			...
			table.on('tool(user2)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
				...
				if (layEvent === 'del') { //删除
					...
					var url = "Purchase/showKinds.action";
					var datas = {
						'purcId' : data.purcId
					};
					$.post(url, datas, function(returnData) {
						...
						$.each(returnData, function(i, item) {
							tabVal += '' + '<tr>' + '<td>' + item.PURC_NAME
									+ '</td>' + '<td>' + item.KIN_NAME
									+ '</td>' + '<td>' + item.KIN_PRICE
									+ '</td>' + '<td>' + item.PUR_TOTAL_NUMBER
									+ '</td>' + '<td>' + item.PUR_TOTAL_PRICE
									+ '</td>' + '</tr>' + '';
						})
						layer.open({
		...
		function addOrUpdata2(title, purcId) {
			layer.open({
				type : 2,
				content : 'page/purchaseForms.jsp?id=' + purcId,
				area : [ '600px', '400px' ],
				title : title,
				cancel : function(index, layero) {
		...
		function add2(obj) {
			var addCen = layer.open({
				...
				shadeClose : true//点击也能遮罩层关闭
				,
				content : "page/purchaseForms.jsp?purcId=" + obj.purcId
						+ "&stId=" + obj.stId
			});
		}
	</script>
