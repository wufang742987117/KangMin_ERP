...
<html>
	...
	<body>
		...
		<table class="layui-table" lay-data="{height:400,size:'sm',skin:'row ', url:'sctockmp/findAll.action',method:'post',page:true,limit:10, id:'tb'}" lay-filter="tb">
		  <thead>
		    <tr>
		      ...
		      <th lay-data="{field:'annex_name', sort: true, width:127}">分店名称</th>
		      <th lay-data="{field:'MEMBER_ID', sort: true,width:130,align:'center',templet:'#memberIdTpl'}">会员编号</th>
		      <th lay-data="{field:'SALE_wholesale_state', sort: true,width:110,align:'center',templet:'#stateTpl'}">销售状态</th>
		      <th lay-data="{field:'sta_name', sort: true,width:95,align:'center'}">售货员</th>
		      <th lay-data="{field:'SALE_NUM', sort: true,width:120,align:'center'}">销售总数量</th>
		      <th lay-data="{field:'SALE_MONEY', sort: true,width:120,align:'center'}">原金额(元)</th>
		      <th lay-data="{field:'SALE_DISCOUNT', sort: true,width:100,align:'center',templet:'#discountTpl'}">折扣</th>
		      <th lay-data="{field:'SALE_MONEY1', sort: true,width:130,align:'center'}">应付金额(元)</th>
		      <th lay-data="{field:'SALE_MONEY2', sort: true,width:130,align:'center'}">实付金额(元)</th>
		      <th lay-data="{field:'SALE_MONEY3', sort: true,width:120,align:'center'}">找零(元)</th>
		      <th lay-data="{fixed: 'right', width:195, align:'left', toolbar: '#barDemo1'}">操作</th>
		    </tr>
		  </thead>
		</table>
		...
		<div id="feedBackList" style="display: none;padding: 5px;">
			<input type="hidden" id="saleId" />
			<table id="feedBackTable" class="layui-table" lay-size="sm">
			  <thead>
			    <tr>
			      <th width="60px">时间</th>
			      <th>原因</th>
			...
			<button class="layui-btn" style="position:absolute;bottom:5px;width:99%;">重 新 申 请</button>
		</div>
	</body>	
	<script type="text/html" id="barDemo1">
		{{#  if(d.SALE_wholesale_state==0) { }}
			<a class="layui-btn layui-btn-mini" lay-event="show"><i class="layui-icon">&#xe63c;</i>查看明细</a>
			<a class="layui-btn layui-btn-warm layui-btn-mini" lay-event="feedBack"><i class="layui-icon">&#xe6b2;</i>打回原因</a>
		{{#  } else { }}
			<a class="layui-btn layui-btn-mini" lay-event="show"><i class="layui-icon">&#xe63c;</i>查看明细</a>
		{{#  } }}
	...
	<script type="text/html" id="stateTpl">
		...
		{{#  } else if(d.SALE_wholesale_state==1) { }}
			<span class="layui-badge layui-bg-orange">批发员审核中</span>
		{{#  } else if(d.SALE_wholesale_state==2) { }}
			<span class="layui-badge layui-bg-orange">财务审核中</span>
		{{#  } else if(d.SALE_wholesale_state==3) { }}
			<span class="layui-badge layui-bg-orange">经理审核中</span>
	...
	<script type="text/javascript">
		...
		layui.use('table', function(){
		  table = layui.table;
			...
			table.on('tool(tb)', function(obj){//tb是table原始容器的属性 lay-filter="对应的值"
				...
				if(layEvent === 'feedBack'){ //查看打回原因
					$("#saleId").val(formdata.SALE_ID);
					//查找打回原因
					var url = "auditAction/showListById.action";
					var data ={"purcId":formdata.SALE_ID};
					$.post(url,data, function(info){
						$("#feedBackList tbody").html("");
						$(info).each(function(i,element){
							$("#feedBackList tbody").append('<tr><td><span class="layui-badge layui-bg-gray">'+element.audTime+'</span></td><td>'+element.feedBack+'</td></tr>');
						});
		...
		$("#feedBackList button").click(function(i,element){
			location.href="sctockmp/sellEntrance.action?saleId="+$("#saleId").val();
		});
