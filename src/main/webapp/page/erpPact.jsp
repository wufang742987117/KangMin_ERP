...
<table class="layui-table" lay-data="{width: 910 , height:495, url:'pact/showList.action', page:true,id:'testReload',method:'post'}" lay-filter="demo">
  <thead>
    <tr>
      ...
      <th lay-data="{field:'pactTitle', width:130, templet: '#usernameTpl'}">合同标题</th>
...
<script type="text/html" id="usernameTpl">
  <a class="layui-table-link" onclick="show('{{d.pactId}}');">{{ d.pactTitle }}</a>
</script>
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('真的删除行么', function(index){
		...
		layer.open({
			...
			content:'page/Pactadd.jsp?pactId='+data.pactId+"&purId="+data.purId+"&applyId="+data.applyId+"&patypeId="+data.patypeId,
			area: ['80%', '80%'],
			title: '编辑合同',
		});	
...
function show(id){
	layer.open({
		type:2,
		skin: 'layui-layer-molv',//样式
		content:'page/erpPactshow.jsp?pactId='+id,
		area: ['60%', '80%'],
		title: '查看合同',
	});
